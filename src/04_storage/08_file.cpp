...
int file_c::open(char const *path, char flag)
{
    ...
    {
        m_fd = ::open(path, O_WRONLY | O_CREAT | O_TRUNC, 0664);
    }
    else
    {
...
// 读取文件内容
int file_c::read(void *buf, sssize_t count) const
{
    // 检查文件描述符
    if (m_fd == -1)
    {
    ...
    // 读取给定字节数
    sssize_t bytes = ::read(m_fd, buf, count);
    if (bytes == -1)
    {
        logger_error("call read fail: %s", strerror(errno));
        return ERROR;
    }
    if ((sssize_t)bytes != count)
    {
...
// 写入文件
int file_c::write(void const *buf, sssize_t count) const
{
    // 检查文件描述符
    if (m_fd == -1)
    {
