...
conn_c::conn_c(char const *destaddr, int ctimeout /* = 30 */,
               int rtimeout /* = 60 */) : m_ctimeout(ctimeout),
                                          m_rtimeout(rtimeout), m_conn(nullptr)
{
    // 检查目的地址
                     ...
                     char **filedata, long long *filesize)
{
    ...
    // 构造请求
    long long bodylen = APPID_SIZE + USERID_SIZE + FILEID_SIZE + sizeof(offset) + sizeof(size);
    long long requlen = HEADLEN + bodylen;
    char requ[requlen];
    ...
    llton(size, requ + HEADLEN + APPID_SIZE + USERID_SIZE + FILEID_SIZE + sizeof(offset));
